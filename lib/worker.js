...
FiveBeansWorker.prototype.doNext = function()
{
	...
	self.client.reserve_with_timeout(10, function(err, jobID, payload)
	{
		if (err)
		{
			if ('TIMED_OUT' !== err)
...
FiveBeansWorker.prototype.callHandler = function(handler, jobID, job, handlerFunction)
{
	...
	var timeoutTime = job._stats ? job._stats.ttr*1000 : 120000;
			...
			job.payload._stats.touch = function() {
	            ...
	            timeout = setTimeout(timeoutFunc, timeoutTime);
	           	self.client.touch(jobID, function(err) {
	                if (err) console.log(err); //self.emit('error', err);
	        	});
			};
		}
