...
FiveBeansWorker.prototype.runJob = function(jobID, job)
{
	...
	{
		if (self.stats.reserves - self.stats.releases >= 2 && self.stats.kicks === 0)
		{
			if (handler.retryFailure) self.callHandler(handler, jobID, job.payload, 'retryFailure');
			else self.buryAndMoveOn(jobID);
		}
		else
		{
			self.callHandler(handler, jobID, job.payload, 'work');
		}
	}
};
...
FiveBeansWorker.prototype.callHandler = function(handler, jobID, jobdata, handlerFunction)
{
	var self = this;
	...
	{
		self.emit('job.timedOut', { id: jobID, handler: handler, payload: jobdata, type: handler.type, priority: self.stats.pri });
	}, self.stats ? self.stats.ttr*1000 - 1000 : 60000);

	try
	{
		handler[handlerFunction](jobdata, function(action, delay)
		{
